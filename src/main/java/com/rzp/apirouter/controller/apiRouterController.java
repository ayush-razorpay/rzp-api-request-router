...
public class apiRouterController {
    ...
    @PostMapping("/webhookProcess/{keyId}")
    ResponseEntity webhookProcess(@RequestBody WebhookDto aDto,
                                  @RequestHeader HttpHeaders headers,
                                  @PathVariable("keyId") String keyId) throws ApiRouterException {

        Map<String, String> headerMap = headers.toSingleValueMap();
        apiRouter.invokeRequest(aDto, headerMap,keyId);
...
//    }
    ...
    ResponseEntity register(@RequestHeader(value="Authorization") String authorization,
                            @RequestBody ApiRouterRequestDto aDto) throws ApiRouterException {
        apiRouter.subscribe(authorization,aDto);
